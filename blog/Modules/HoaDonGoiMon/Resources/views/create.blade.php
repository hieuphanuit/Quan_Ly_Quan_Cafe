	...
	$(document).ready(function() {
		$(".thucDon_btn").on("click", function() {
			...
			if ($('.child-selection[data-id="' + $(this).data('id') + '"]').length) {
				...
				var inputfield = "<tr><td>" + mon + "</td><td><div class='row'><div><button class='btn btn-warning subtract-btn' data-id='" + id + "'>-</button></div><div class='col-md-4'><input class='child-selection form-control counter' id='counter" + id + "'style='width:60px;' type='number' data-id='" + id + "' value='1'/></div><div><button class='btn btn-success plus-btn' data-id='" + id + "'>+</button></div></div></td><td><div><button class='btn btn-danger delete-btn'>x</button></div></td></tr>";
